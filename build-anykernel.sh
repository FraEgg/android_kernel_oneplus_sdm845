...
export CLANG_TRIPLE_ARM32=$CLANG_TRIPLE_ARM32
...
function change_variant {
		...
		sed -i '13s/.*/device.name2='$TAG'/' anykernel.sh
		cd $KERNEL_DIR
}
...
function make_modules {
		...
		if [ "$SIGN_MODULES" == 1 ];then
		  ...
		  $SIGNFILE_DIR/sign-file -dp sha512 \
...
function finished_build {
	...
	echo "Debug Mode Activated: Exit Here:"
	exit
	fi
	TIME_END
	check_filesize
...
options=( 1 "OnePlus 6/6T"
	2 "OnePlus 6/6T (6T Global)"
	3 "OnePlus 6/6T (Nebula2)"
	4 "OnePlus 6/6T (sdm845)"
	5 "Oneplus 6/6T (sdm845-perf)"
        6 "Build All")
